...
SO_NODE_SOURCE(SoXipNeheStarGenerator)

void
SoXipNeheStarGenerator::initClass()
{
   SO_NODE_INIT_CLASS(SoXipNeheStarGenerator, SoXipKit, "Node")
}


SoXipNeheStarGenerator::SoXipNeheStarGenerator()
{
  SO_NODE_CONSTRUCTOR(SoXipNeheStarGenerator)
  
    srand(time(NULL));

    for (int i = 0; i < MAX_STARS; i++)
    {
      _starInfos[i].angle = 0.0f;
      _starInfos[i].distance = (float(i)/MAX_STARS) * 5.0f;
      _starInfos[i].r = rand() % 255 + 1;
      _starInfos[i].g = rand() % 255 + 1;
      _starInfos[i].b = rand() % 255 + 1;

      this->addChild( new SoXipNeheStar() );
    }
